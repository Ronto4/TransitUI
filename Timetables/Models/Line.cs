...
public record Line
{
    public record Route
    {
        ...
        public bool DoesStopAt(Stop stop) => StopPositions.Select(pos => pos.Stop).Contains(stop);
    ...
    public bool DoesStopAt(Stop stop) => Routes.Any(route => route.DoesStopAt(stop));
